...
LDFLAGS := -framework Accelerate -framework Foundation -framework IOSurface -framework Metal -framework MetalPerformanceShaders -framework QuartzCore
...
$(BUILD_DIR)/npu_bench.o: npu_bench.mm npu_bench.h | $(BUILD_DIR)
	$(CXX) $(CXXFLAGS_MM) -c $< -o $@
